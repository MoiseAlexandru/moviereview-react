...
export default function App() {
    ...
    function handleAddWatched(newMovie) {
        /*
        const alreadyExisting = watched.filter((movie) => (movie.imdbID === newMovie.imdbID));
        if(alreadyExisting.length === 0)
            setWatched((watched) => [...watched, newMovie]);
        else
        {
            const newWatchedList = watched.map((movie) => (movie.imdbID === newMovie.imdbID ? {...movie, userRating: newMovie.userRating} : movie));
...
function MovieList({movies, onSelectMovie}) {
    return (
        <ul className="list list-movies">
            {movies?.map((movie) => <Movie movie = {movie} key={movie.imdbID} onSelectMovie = {onSelectMovie}></Movie>)}
        </ul>
    );
}
